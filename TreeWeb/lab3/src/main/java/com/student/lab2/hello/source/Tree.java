package com.student.lab2.hello.source;

import java.util.*;

public class Tree <K, V> implements Map<K, V> {
    ...
    public int size() {
        int c = 0;
        if(root != null) {
            QueueTree<TreeNode> q = new QueueTree<TreeNode>();
            q.add(root);
            while(!q.isEmpty()) {
    ...
    public String printTree() {
        ...
        for(Object[] line : lines) {
            ...
            for(Object o : line) {
                int of2 = (w*6/of-3);
                if(of2 < 1)
                    out.append(String.format("%6s", o == null ? " " : o));
                else
                    out.append(String.format("%"+of2+"s"+"%6s%"+of2+"s", " ", o == null ? " " : o, " "));
            }
            l++;
            out.append("\n").append(d);
        }
    ...
    public Set<Entry<K, V>> entrySet() {
        Collection<Entry<K, V>> list = loop();
        LinkedHashSet<Entry<K, V>> set = new LinkedHashSet<>(list);
        return Collections.unmodifiableSet(set);
    }
